...
void solve()
{
    ...
    vector<pair<ll,ll>>knight_pos;
    for(int i=0;i<n;i++)
    {
        pair<ll,ll>p;
        cin >> p.first >> p.second;
        
        knight_pos.push_back(p);
    ...
    for(int i=0;i<n;i++)
    {
        knight_pos[i].first -= x;
        knight_pos[i].second -= y;
    ...
    for(int i=-1;i<=1;i++)
    {
        ...
        {
            for(int k=0;k<n;k++){
	                if(( abs(knight_pos[k].first-i)==2 && abs(knight_pos[k].second-j)==1) || ( abs(knight_pos[k].first-i)==1 && abs(knight_pos[k].second-j)==2)){
	                    check++;
	                    break;
	                }
	            }
